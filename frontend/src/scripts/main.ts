document.addEventListener('DOMContentLoaded', () => { // Espera a que el DOM esté cargado
  ...
  if (app) {                                  // y coloca un mensaje de estado inicial.
...
interface Film {                    // Aun así, más abajo se usa Record<string, any>[]
...
const loadFilms = async () => {
  try {               //Realiza un fetch HTTP GET a /api/films
    ...
    renderPagination();
  } catch (error) {         // Captura el error, de haberlo
...
const renderFilms = () => {   // Se declara una función anónima de flecha
  ...
  const buttons = `
    <div class="mb-3 d-flex align-items-center flex-wrap gap-2">
      <button id="btn-all-films" class="btn btn-primary btn-lg rounded-circle">Todas las películas</button>
      <button id="btn-available-films" class="btn btn-warning btn-lg rounded-circle">TOP películas</button>
      <div class="input-group" style="width: 350px;">
        <input type="text" id="search-input" class="form-control btn-lg" placeholder="Buscar por título o descripción...">
        <button class="btn btn-primary btn-lg" type="button" id="filter-button">
          Filtrar
        </button>
        <button class="btn btn-warning btn-lg" type="button" id="clear-search">
          Limpiar
        </button>
      </div>
...
const renderPagination = () => {
  ...
  allButtons.forEach(btn => {
    btn.addEventListener('click', () => { // Se les asigna un evento click
